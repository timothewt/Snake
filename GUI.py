...
class GUI:
    def __init__(self):
        self.snake = Snake(head_y=7, head_x=5, apple_position=Coordinates(7, 10))
    ...
    def draw_game_grid(self):
        ...
        [pg.draw.rect(self.screen, (0, 200, 0), pg.Rect(body_part.x * 40, body_part.y * 40, 40, 40)) for body_part in self.snake.body]
        pg.display.update()
    ...
    def draw_score_bar(self):
        ...
        pg.draw.rect(self.screen, (50, 30, 60), pg.Rect(0, settings.X_SIZE * 40, settings.Y_SIZE * 40, 40))
        self.screen.blit(text, (10, settings.Y_SIZE * 40 + 8))
        pg.display.update()
    ...
    def draw_controls(self) -> None:
        ...
        """
        x = settings.X_SIZE * 20 - 78  # or * 40 // 2
        y = settings.Y_SIZE * 80 // 3  # or * 40 * 2) // 3
        # left
    ...
    def draw_game_over(self) -> None:
        ...
        x = (settings.X_SIZE + 1) * 20 - text.get_width() // 2
        y = (settings.Y_SIZE + 1) * 20 - text.get_height() // 2
        self.screen.blit(text, (x, y))
        x_sub = (settings.X_SIZE + 1) * 20 - text_sub.get_width() // 2
        y_sub = y + size + 15
    ...
    def on_key_pressed(self, key_pressed: int):
        ...
        if key_pressed not in self.directions.keys():
            return False
        self.snake.pick_new_direction(self.directions[key_pressed])
        return True
